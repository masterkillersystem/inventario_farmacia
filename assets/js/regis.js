...
function inviModal1(item_id){
	...
	$.ajax({
			...
			success: function (data) {
				...
				$('#modal-invi').find('.modal-title').text("Agregar al inventario");
				$('#modal-invi').modal('show');
			},
			error: function(){
				alert('Error: L56+');
			}
...
$('#del-item').click(function(event) {
	...
	var check = 0;
	 ...
	 if(check == 0){
	 	alert('Porfavor Seleccione Fila!');
	 }else{
	 	$('#confirm-type').val();
...
//del from item
function del_item(item_id,item_type_id){
	$.ajax({
			url: 'data/del_item.php',
			type: 'post',
			data: {
				item_id:item_id,
