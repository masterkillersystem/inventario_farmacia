...
class Stock extends Database implements iStock {
	...
	public function all_stockList()
	{
		/*$sql = "SELECT *
			...
			$sql = "SELECT * FROM item, stock,  item_type where item_type.item_type_id=item.item_type_id and item.item_id=stock.item_id and stock.stock_qty!=? GROUP BY stock.stock_id  ORDER BY item.item_name ASC ";	
	...
	{
		$sql = "SELECT s.stock_id, i.item_id, i.item_name, i.item_code, i.item_grams, i.item_compra, i.item_price,SUM(stock_qty) as qty FROM stock s INNER JOIN item i ON s.item_id = i.item_id GROUP BY s.stock_id ORDER BY i.item_name ASC";
